...
<body>
    ...
    <p class="meta">Total data: {{ $records->count() }} pengguna</p>

    <div class="filters">
        <p class="filters-title">Ringkasan Filter</p>
        <p><strong>Pencarian:</strong> {{ $filters['search'] ?? '-' }}</p>
        <p><strong>ID Peran:</strong> {{ $filters['role_id'] ?? '-' }}</p>
    </div>
        ...
        <tbody>
            @forelse ($records as $user)
                <tr>
                    ...
                    <td>{{ (int) $user->roles_count }}</td>
                    <td class="muted">{{ optional($user->created_at)->format('d-m-Y H:i') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="muted">Tidak ada data pengguna sesuai filter.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
