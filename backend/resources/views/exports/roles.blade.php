...
<body>
    ...
    <p class="meta">Total data: {{ $records->count() }} peran</p>

    <div class="filters">
        <p class="filters-title">Ringkasan Filter</p>
        <p><strong>Pencarian:</strong> {{ $filters['search'] ?? '-' }}</p>
        <p><strong>ID Hak Akses:</strong> {{ $filters['permission_id'] ?? '-' }}</p>
    </div>
        ...
        <tbody>
            @forelse ($records as $role)
                <tr>
                    ...
                    <td class="muted">{{ optional($role->created_at)->format('d-m-Y H:i') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" class="muted">Tidak ada data peran sesuai filter.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
